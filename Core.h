	...
	void createRootSignature()
	{
		...
		if (FAILED(hr))
		{
			if (error)
			{
				...
				printf("RootSignature serialize error: %s\n", (const char*)error->GetBufferPointer());
				error->Release();
			}
			else
			{
				printf("RootSignature serialize failed with no error blob. hr=0x%08x\n", hr);
			}
			return;
		}
		//MessageBoxA(NULL, (const char*)error->GetBufferPointer(), "RootSignature Error", 0);
