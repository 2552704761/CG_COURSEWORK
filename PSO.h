...
public:
    ...
    ID3D12PipelineState* psoSkybox = nullptr;
    void createPSO(Core* core, std::string name, ID3DBlob *vs, ID3DBlob *ps, D3D12_INPUT_LAYOUT_DESC layout)
    {
        ...
        if (FAILED(hr))
        {
            printf("Failed to create PSO: %s\n", name.c_str());
            return;
        }
        psos.insert({ name, pso });
    }
    void createPlayerPSO(Core* core, std::string name, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_LAYOUT_DESC layout)
    {
        if (psos.find(name) != psos.end())
        {
            return;
    ...
    void createSkyPSO(Core* core, std::string name, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_LAYOUT_DESC layout)
    {
        ...
        if (FAILED(hr))
        {
            printf("PSO creation failed: %s\n", name.c_str());
            return;
        }
        psos.insert({ name, pso });
    }
    void createLinePSO(Core* core, std::string name, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_LAYOUT_DESC layout)
    {
        if (psos.find(name) != psos.end())
        {
            return;
